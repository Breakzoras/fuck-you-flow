...
cd "$(dirname "$0")/.." || exit 1
taskkill //IM fuckyouflow.exe //F >/dev/null 2>&1
...
WAIT=${1:-75}
sleep "$WAIT"
tasklist | grep -i "fuckyouflow.exe\|whisper-server" | head -3
LOG=$(ls -t "$LOCALAPPDATA"/FuckYouFlow/logs/fuckyouflow.log.* | head -1)
